...
var accordion = function accordion(triggersSelector) {
  var btns = document.querySelectorAll(triggersSelector); // С помощью JS
  ...
  // });
  // btns.forEach(btn => {
  //     btn.addEventListener('click', function() { //Если у елемерта по которому кликнули нет класса активности,
  //         if (!this.classList.contains('active')) { // то мы ему добавим класс активности а у остальных уберем. 
  //             btns.forEach(btn => {
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = "Пожалуйста, виберите размер и материал картины";
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  function showSlides(n) {
    if (n > items.length) {
      // когда доходит до последнего слайда
      slideIndex = 1; // возвращается на первый
    }

    if (n < 1) {
      // в обратном порядке, когда стает меньше первого
      slideIndex = items.length; // начинается с последнего
    }

    items.forEach(function (item) {
      item.classList.add("animated"); //скрываем все неактивные слайды

      item.style.display = "none";
    });
    items[slideIndex - 1].style.display = 'block'; // и показываем только первый слайд
  }
